/**
 * AQA's GCSE science specifications, as their own numbered subject content.
 *
 * The three separate sciences and the Combined Science: Trilogy course that
 * shares most of their content. Trilogy is written out in full rather than
 * derived from the separate three: the board publishes it as its own document
 * with its own numbering, and a question tagged against it must carry ids a
 * Trilogy student would recognise.
 *
 * See `exam-specification-outlines.ts` for the shape and how ids are derived.
 */

import type { SpecificationOutline } from "@/lib/practice/exam-specification-outlines";
import { CHECKED_SOURCE } from "@/lib/practice/specifications/checked-source";
