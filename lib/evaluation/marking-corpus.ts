...
export const MARKING_CORPUS_SOURCES: readonly MarkingCorpusSource[] = [
  ...
  },
  {
    ...
    commentary: false,
    notes:
      "The most valuable source in the corpus for its size. Ingested: 480 answers over 20 questions, half typed and half photographed handwriting, every one marked by two examiners independently. They disagree on 237 of 480, and the gap widens with the tariff — a mean of 0.05 marks on one-markers against 4.58 on the 40-mark essays, worst case 20. That curve is the bar Jami should be held to, because holding it to exact agreement would hold it to a standard the examiners themselves do not meet. No written examiner commentary. " +
      "Provenance, checked rather than assumed: these are GCSE *mock* exam questions, not awarding-body past papers. The dataset's own README calls it a double-marked GCSE mock-exam dataset, and no question file names a board — searching every question for Edexcel, AQA, OCR, Pearson or WJEC returns nothing. The mark schemes are real schemes written for those mock questions in the boards' own notation, which is why they read as official; the students and both examiners are real. So it can measure whether a scheme was applied correctly, and it cannot by itself establish readiness on authentic past-paper wording, which is what Past Paper Practice serves. " +
      "Licence scope: CC BY 4.0, with the full text shipped in the payload. Share and adapt for any purpose including commercially, with attribution to Fox et al., 2026. That covers evaluation and redistribution alike. " +
      "Two things in the source that the ingest does not carry. Sixty answers (english_07 and english_08, both 40-mark) record each examiner's split across assessment objectives in dataset.csv, and those criterion-level marks are dropped on the way in — recovering them would be the corpus's only criterion-level GCSE evidence. And the maths images are the student's stroke canvas only, cropped to their strokes: the README warns they can contain a stray line, circle, underline or tick left over from annotating question text that is not in the image. Those are the student's marks, not an examiner's, and a leak check that reads a circled numeral as an examiner annotation will misfire on them.",
  },
  {
