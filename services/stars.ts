...
import { resolveStarPresetId } from "@/lib/stars";
import type { NormalizedStar } from "@/lib/stars";
import { withTimeout } from "@/services/firestore";

const QUERY_MS = 30_000;
const CREATE_MS = 30_000;
const UPDATE_MS = 30_000;
const STAR_COUNT_FIELD = "awardedStarsCount";
...
export async function createStarForGoalIfMissing(userId: string, goal: Goal) {
  ...
  const star = {
    ...
    size: Math.log(Math.max(0, goal.targetCards) + 1),
