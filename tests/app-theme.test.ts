...
/**
 * A notebook page is meant to sit above the desk behind it rather than be drawn
 * onto it. Both halves of that are themed, and the failure is silent: a black
 * page on a near-black desk with the wrong rim is simply invisible, and nothing
 * about the app being broken shows up anywhere else.
 */
describe("the notebook page's lift", () => {
  it("is defined once, and reached rather than repeated", () => {
    expect(globalsCss).toMatch(/--notebook-page-lift:\s*0 \d+px \d+px/);
    expect(globalsCss).toMatch(/--notebook-page-edge:\s*rgba/);

    // The sheet asks for the token; it does not carry a value of its own.
    expect(globalsCss).toContain("box-shadow: var(--notebook-page-lift)");
    expect(globalsCss).toContain("border-color: var(--notebook-page-edge)");
  });

  it("turns the rim over on a pale desk", () => {
    // On a dark desk the page edge catches light; on a pale one a white
    // hairline on white paper is three shades of nothing, so it goes dark.
    const light = globalsCss.slice(globalsCss.indexOf("body.app-theme-light {"));
    expect(light.slice(0, 240)).toMatch(/--notebook-page-edge:\s*rgba\(\s*15/);

    // And only the two themes that are actually pale get that override.
    expect(getActiveAppThemeClassNames("paper-white")).toContain("app-theme-light");
    expect(getActiveAppThemeClassNames("pink")).toContain("app-theme-light");
    for (const dark of ["normal", "purple", "soft-grey", "black"] as const) {
      expect(getActiveAppThemeClassNames(dark), dark).not.toContain("app-theme-light");
    }
  });

  it("does not paint under the sheets waiting either side of a swipe", () => {
    // A shadow falls outside the box, and hiding an element does not clip it,
    // so the next page's shadow would creep in at the edge of the current one.
    const preview = globalsCss.slice(
      globalsCss.indexOf(".notebook-page-swipe-preview {")
    );
    expect(preview.slice(0, 120)).toContain("box-shadow: none");
  });
});
