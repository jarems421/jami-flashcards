...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => ({})) as { manifest?: ExamPaperIngestionManifest; dryRun?: boolean; captureExtraction?: boolean };
  if (!body.manifest) return Response.json({ error: "manifest_required" }, { status: 400 });
  try { return Response.json(await ingestExamPaper(body.manifest, { dryRun: body.dryRun === true, captureExtraction: body.dryRun === true && body.captureExtraction === true })); }
