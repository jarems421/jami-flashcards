...
export async function POST(request: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    transaction.update(jobRef, { status: "approved", drafts: [], evidence: [], approvedCardIds: cards.map((card) => card.id), completedAt: now, expiresAt: Timestamp.fromMillis(now + 24 * 60 * 60_000), updatedAt: now });
    return { kind: "created" as const, cards };
  });
  ...
  if (result.kind === "created") return Response.json({ cards: result.cards });
