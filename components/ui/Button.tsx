...
const variantClasses: Record<ButtonVariant, string> = {
  ...
  ghost:
    "border border-transparent bg-transparent text-text-secondary hover:bg-[var(--button-ghost-bg-hover)] hover:text-text-primary active:scale-[0.98]",
  surface:
  ...
  warm:
    "border border-[var(--button-warm-border)] bg-[var(--button-warm-bg)] text-[var(--button-warm-text)] shadow-[var(--button-warm-shadow)] hover:-translate-y-[1px] hover:border-[var(--button-warm-border-hover)] hover:bg-[var(--button-warm-bg-hover)] hover:shadow-[var(--button-warm-shadow-hover)] active:translate-y-0 active:scale-[0.98]",
};
...
export default function Button({
  ...
  return (
    <button
      className={`relative inline-flex items-center justify-center overflow-hidden rounded-[2rem] font-medium tracking-[0.01em] transition duration-fast ease-spring disabled:cursor-not-allowed disabled:shadow-none disabled:saturate-[0.72] ${variantClasses[variant]} ${sizeClasses[size]} ${className}`}
      {...props}
    >
      {children}
    </button>
